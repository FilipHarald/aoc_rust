...
fn move_and_deliver(mut x: &i32, mut y: &i32, c: char, mut vistited_houses: &HashMap<Vec<i32>, i32>) {
        match c {
            '^' => *x += 1,
            'v' => *x -= 1,
            '>' => *y += 1,
...
fn solve_b(input: &str) -> usize {
    ...
    for (index, c) in input.chars().enumerate() {
        if index % 2 == 0 {
            move_and_deliver(&santa_x, &santa_x, c, &vistited_houses);
        } else {
            move_and_deliver(&robot_x, &robot_y, c, &vistited_houses);
        }
    }
    return vistited_houses.len();
}
